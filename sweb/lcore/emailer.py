"""
performs emailing with Sendgrid
"""
import logging
...
def send_email_old(body, destination, subject):
    ...
    try:
        client = SendGridAPIClient(getattr(settings, "EMAIL_KEY", None))
...
def email_main(existing_user, **email_kwargs):
    '''  Main code to build the components from key, invitee, user_name, 
        group_name, destination, subject     '''
